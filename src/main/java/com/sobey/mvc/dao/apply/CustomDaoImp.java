...
public class CustomDaoImp implements CustomDao {
	...
	public List findComputeStorageItemListByApply(Integer applyId) {
		String sql = "SELECT  s.id AS sid,s.identifier AS sIdentifier,s.storage_space ,GROUP_CONCAT(c.id SEPARATOR '/') AS cid ,GROUP_CONCAT( c.identifier SEPARATOR ' , ') AS cIdentifier FROM apply a LEFT JOIN storage_item s ON a.id = s.apply_id LEFT JOIN compute_storage_item cs ON s.id = cs.storage_item_id  RIGHT JOIN compute_item c ON cs.compute_item_id = c.id WHERE a.id = ? GROUP BY s.id";
