...
import com.sobey.cmop.mvc.dao.apply.ApplyDao;
import com.sobey.cmop.mvc.dao.apply.ComputeItemDao;
import com.sobey.cmop.mvc.dao.apply.CustomDao;
import com.sobey.cmop.mvc.dao.apply.FaultDao;
import com.sobey.cmop.mvc.dao.apply.StorageItemDao;
...
import com.sobey.cmop.utils.MailUtil;
import com.sobey.cmop.utils.RedmineUtil;
...
public class ApplyManager {
	...
	@Transactional(readOnly = false)
	public void saveES3(String[] ecsIds, String[] spaces, String[] storeageTypes, Apply apply) {
		DateTime dateTime = new DateTime();
		Subject subject = SecurityUtils.getSubject();
		User user = userDao.findByEmail(subject.getPrincipal().toString());// 当前用户
		apply.setUser(user);
		...
		for (int i = 0; i < spaces.length; i++) {
			...
			storageItem.setStorageType(Integer.parseInt(storeageTypes[i]));
	...
	@Transactional(readOnly = false)
	public void updateES3(String[] ecsIds, String[] spaces, String[] spaceIds, String[] storeageTypes, Apply apply) {
		Subject subject = SecurityUtils.getSubject();
		User user = userDao.findByEmail(subject.getPrincipal().toString());// 当前用户
		apply.setUser(user);
		apply.setStatus(1);
	 ...
	 * 
	 * <pre>
	 * 显示页面字段为:   存储空间ID,存储空间identifier,存储大小,存储类型,计算资源ID,计算资源identifier
	 * </pre>
	 * 
	 * @param applyId
	 * @return
