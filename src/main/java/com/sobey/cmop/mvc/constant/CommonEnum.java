 ...
 * 实现该方法后,需要声明一个private Map集合,将enum中的值迭代至Map集合中,key为enum的输入参数.value为enum的值. <br>
 * 最后用Map集合的get方法获得获得value
 * <p>
 * eg: <br>
 * <code>                                                                                                       
 * public enum EnumTest{ <br>                                                                                   
 * 		ONE,TWO; <br>                                                                                           
 * 		private static final Map<Integer, String> lookup = Maps.newHashMap();<br>                               
 * 		static {<br>                                                                                            
 * 			for (EnumTest enumTest : EnumTest.values()) {<br>                                                   
 * 				lookup.put(enumTest.code, enumTest.name());<br>                                                 
 * 			}<br>                                                                                               
 * 		}<br>                                                                                                   
 * public String get(Integer code) {<br>                                                                  
 * 		return lookup.get(code);<br>                                                                        
 * }<br>                                                                                                       
 * }<br>                                                                                                        
 * EnumTest.get(1) = ONE                                                                                        
 * </code>
