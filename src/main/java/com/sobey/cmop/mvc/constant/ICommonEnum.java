 ...
 * 
 * <strong> 注意:一定要在Enum内写一个static的get方法 </strong> 该方法根据传入的参数,返回enum中的文本.
 * 实现该方法后,需要声明一个 Map集合,将enum中的值迭代至Map集合中,key为enum的输入参数.value为enum的值.
...
public interface ICommonEnum {
	...
	/**
	 * 在该方法中返回从构造函数中传入的参数,并且参数是Integer类型
