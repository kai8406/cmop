<%@ page contentType="text/html;charset=UTF-8"%>
<%@ include file="/WEB-INF/layouts/taglib.jsp"%>
<html>
<head>
<title>基础数据-IP池管理</title>
<script>
	$(document).ready(function() {
		$("ul#navbar li#basicdata").addClass("active");
		$("div#leftbar li span").tooltip();
		$("#ip-bar").addClass("active").find("span").addClass("txt-color");
	});
</script>
</head>

<body>

	<c:if test="${not empty message}"><div id="message" class="alert alert-success fade in"><button data-dismiss="alert" class="close" type="button">×</button><span>${message }</span></div></c:if>
	<c:if test="${not empty errorMessage}"><div id="errorMessage" class="alert alert-danger fade in"><button data-dismiss="alert" class="close" type="button">×</button><span>${errorMessage}</span></div></c:if>
	<div class="row" >
	
		<div id="leftbar" class="span2">
			<ul class="nav nav-list" style="width: 120px;">
				<li id="location-bar"><a href="${ctx}/basicdata/location/"><i class="icon-inbox"></i><span data-placement="right" title="IDC管理">IDC管理</span></a></li>
				<li id="vlan-bar"><a href="${ctx}/basicdata/vlan/"><i class="icon-inbox"></i><span data-placement="right" title="VLAN管理">VLAN管理</span></a></li>
				<li id="ip-bar"><a href="${ctx}/basicdata/ippool"><i class="icon-list" ></i><span data-placement="right" title="IP创建、查询、修改、删除">IP管理</span></a></li>
				<li id="excel-bar"><a href="${ctx}/basicdata/import"><i class="icon-edit" ></i><span data-placement="right" title="已用数据初始化导入">Excel导入</span></a></li>
				<li id="hostServer-bar"><a href="${ctx}/basicdata/host/"><i class="icon-inbox" ></i><span data-placement="right" title="宿主机及物理机的创建、查询及管理">服务器管理</span></a></li>
			</ul>
		</div>
	
		<div id="main" class="span10">	
		
			<form class="form-inline well well-small" action="#">
				<div class="row">
					<div class="span3">
						<label class="control-label search-text">IP地址:</label> 
						<input type="text"  name="search_LIKE_ipAddress" class="input-small" maxlength="45"  value="${param.search_LIKE_ipAddress}">
					</div>
					
					<div class="span3">
						<label class="control-label">IP池类型:</label> 
						<select name="search_EQ_poolType" class="input-medium">
							<option value="">全部</option>
							<c:forEach var="map" items="${poolTypeMap }">
								<option 
									<c:if test="${map.key == param.search_EQ_poolType }"> selected="selected"</c:if>
										value="${map.key }"><c:out value="${map.value }" />
								</option>
							</c:forEach>
						</select>
					</div>
								
					<div class="span3">
						<label class="control-label">IP状态:</label> 
						<select name="search_EQ_status" class="input-small">
							<option value="">全部</option>
							<c:forEach var="map" items="${ipStausMap }">
								<option 
									<c:if test="${map.key == param.search_EQ_status }">	selected="selected"</c:if>
										value="${map.key }"><c:out value="${map.value }" />
								</option>	
							</c:forEach>
						</select>
					</div>		
					
					<div class="span2 pull-right">
						<button class="btn" type="submit"><i class="icon-search"></i>查询</button>
						<button class="btn tip-bottom reset" title="刷新" type="reset"><i class="icon-refresh"></i></button>
					</div>
				</div>
			</form>
		
			<table class="table table-striped table-bordered table-condensed">
				<thead>
					<tr>
						<th>IP地址</th>
						<th>IP池类型</th>
						<th>状态</th>
						<th>VLAN</th>
						<th>IDC</th>
						<th>操作</th>
					</tr>
				</thead>
				<tbody>
					<c:forEach items="${page.content}" var="item">
						<tr>
							<td>${item.ipAddress}</td>
							<td>
								<c:forEach var="map" items="${poolTypeMap }">
								<c:if test="${map.key == item.poolType}">
									<c:out value="${map.value }"/>
								</c:if>
								</c:forEach>
							</td>
							
							<td>
								<c:if test="${item.status == 1 }">
									<span class="label label-success">未使用</span>
								</c:if> 
								<c:if test="${item.status == 2 }">
									<span class="label label-important">已使用</span>
								</c:if> 
							</td>
							
							<td>
								<c:forEach var="vlan" items="${vlanList}">
									 <c:if test="${vlan.id == item.vlan.id}">${vlan.name }</c:if>
								</c:forEach>
							</td>
			
							<td>
								<c:forEach var="location" items="${locationList }">
									<c:if test="${location.id == item.vlan.location.id}">${location.name }</c:if>
								</c:forEach>
							</td>
								
							<td>
								<a	href="${ctx }/basicdata/ippool/update/${item.id}">修改</a>
								<c:if test="${1 == item.status}">
									<a data-toggle="modal" href="#deleteModal${item.id}">删除</a>
									<div id="deleteModal${item.id }" class="modal hide fade form-horizontal">
										<div class="modal-header">
											<button data-dismiss="modal" class="close" type="button">×</button>
											<strong>提示</strong>
										</div>
										<div class="modal-body" >是否删除?</div>
										<div class="modal-footer">
											<a class="btn" data-dismiss="modal" href="#">关闭</a>
											 <a href="${ctx}/basicdata/ippool/delete/${item.id}" class="btn btn-primary">确定</a>
										</div>
									</div>
								</c:if>
							</td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
		
			<tags:pagination page="${page}" />
		
			<a class="btn" href="${ctx}/basicdata/ippool/save/">创建IP</a>
		
		</div>
	</div>
	
</body>
</html>
