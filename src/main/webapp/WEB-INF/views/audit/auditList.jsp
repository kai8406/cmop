	...
	<form class="well well-small form-search" action="${ctx}/audit/">
		<div class="row-fluid show-grid">
			<div class="span4">
				...
				<input type="text" id="title" name="title" placeholder="标题" class="input-medium">
			</div>
			
			<div class="span4">
				<label class="control-label">审核状态:</label> 
				<select id="status" name="status" class="input-mini">
		...
		<tbody>
			<c:forEach items="${page.content}" var="apply">
				<tr>
					...
					<td> 
						<a href="${ctx}/audit/view/${apply.id}">查看</a>
						<c:if test="${apply.status!=4}">
							<a href="${ctx}/audit/create/${apply.id}">审批</a>
						</c:if>					
					</td>	
				</tr>
			</c:forEach>
